...
public class AI extends Player  {
	...
	public AI(String username, Avatar avatar, GameSession gameReference) {
		super(username, avatar, gameReference);
	...
	public void printBoard() {
	    ...
	    System.out.print(
	        "\n\nThere are " + gameReference.getBag().getRemainingCount() + " tiles remaining in the bag.");
	}
	...
	public void setbestmoves(){
		...
		for (int i=0;i< words.size();i++) {
			...
			HashMap<String, ArrayList<String>> map = DataHandler.getBitOptions(s);
			ArrayList<String> prefixe = map.get("Prefixes");
			ArrayList<String> suffixe = map.get("Suffixes");
			...
			for (int j=0; j<prefixe.size();j++) {
				...
				if(wort.getColumn()) {
					...
					if(checkabove(laengepre.length, wort, this.gameReference.getBoard()) && checkabove(laengesuf.length, wort, this.gameReference.getBoard())) {
						placedsquare = checktilesvertical(buchstaben, new ArrayList<Square>(), tilescopy, wort, laengepre.length, laengesuf.length, this.gameReference.getBoard());
						
						
						if(placedsquare!=null) {
							for (Square x: placedsquare) {
								gameReference.placeTile(x.getX(), x.getY(), x.getTile());
//								System.out.println("Buchstabe: "+String.valueOf(x.getTile().getLetter()+" X: "+x.getX()+" Y: "+x.getY()));
								this.printBoard();
							}
//							System.out.println("Squares");
							...
							if(gameReference.checkMove()) {
								...
								for (Square x: placedsquare) {
									gameReference.recallTile(x.getX(), x.getY());
								}
							}
						}
					...
					ArrayList<Tile> tilescopy = this.copytiles(this.tiles);
					if(checkleft(laengepre.length, wort, this.gameReference.getBoard()) && checkright(laengesuf.length, wort, this.gameReference.getBoard())) {
						placedsquare = checktileshorizontal(buchstaben, new ArrayList<Square>(), tilescopy, wort, laengepre.length, laengesuf.length, this.gameReference.getBoard());
						gameReference.setPlacedSquares(placedsquare);
						if(placedsquare!=null) {
							if(gameReference.checkMove()) {
								PossibleMove pm = new PossibleMove(placedsquare, gameReference.getTurnValue());
								this.moves.add(pm);
	...
	public ArrayList<Square> checktilesvertical(ArrayList<String> buchstaben, ArrayList<Square> placedsquares, ArrayList<Tile> remainingtiles,Word wort, int pre, int suf, Board board){
		...
		if (buchstaben.size()==0) {
			...
			if(buchstaben.size()<=pre){
				if(board.getSquare(wort.getBeginningX(), wort.getBeginningY()+(pre-buchstaben.size()+1)).isTaken()) {
					char[] c = buchstaben.get(buchstaben.size()-1).toCharArray();
					if(board.getSquare(wort.getBeginningX(), wort.getBeginningY()+(pre-buchstaben.size()+1)).getTile().getLetter() == c[0]) {
						buchstaben.remove(buchstaben.size()-1);
						return checktilesvertical(buchstaben, placedsquares, remainingtiles, wort, pre, suf, board);
				...
				if(board.getSquare(wort.getBeginningX(), wort.getEndingY()- suf).isTaken()) {
					char[] c = buchstaben.get(buchstaben.size()-1).toCharArray();
					if(board.getSquare(wort.getBeginningX(), wort.getEndingY()-suf).getTile().getLetter() == c[0]) {
						buchstaben.remove(buchstaben.size()-1);
						suf=suf-1;
						return checktilesvertical(buchstaben, placedsquares, remainingtiles, wort, pre, suf, board);
	...
	public ArrayList<Square> checktileshorizontal(ArrayList<String> buchstaben, ArrayList<Square> placedsquares, ArrayList<Tile> remainingtiles,Word wort, int pre, int suf, Board board){
		...
		if (size==0) {
			...
			if(buchstaben.size()<=pre){
				if(board.getSquare(wort.getBeginningX()+(pre-buchstaben.size()-1), wort.getBeginningY()).isTaken()) {
					char[] c = buchstaben.get(buchstaben.size()-1).toCharArray();
					if(board.getSquare(wort.getBeginningX()+(pre-buchstaben.size()-1), wort.getBeginningY()).getTile().getLetter() == c[0]) {
						buchstaben.remove(buchstaben.size()-1);
						return checktileshorizontal(buchstaben, placedsquares, remainingtiles, wort, pre, suf, board);
				...
				if(board.getSquare(wort.getEndingX()+suf, wort.getEndingY()).isTaken()) {
					char[] c = buchstaben.get(buchstaben.size()-1).toCharArray();
					if(board.getSquare(wort.getEndingX()+suf, wort.getEndingY()).getTile().getLetter() == c[0]) {
						buchstaben.remove(buchstaben.size()-1);
						suf=suf-1;
						return checktileshorizontal(buchstaben, placedsquares, remainingtiles, wort, pre, suf, board);
			...
			for (int i=0;i<this.gameReference.getBoard().getSquareList().size();i++) {
				...
				if(this.gameReference.getBoard().getSquareList().get(i).isTaken()) {
					...
					if(lowerneighbour!=null) {
						if((lowerneighbour.isTaken()) && lowerneighbour!=null && (!this.gameReference.getBoard().getSquareList().get(i).isWihthinColumnWord())) {
							Square upperneighbour = this.gameReference.getBoard().getUpperNeighbour(this.gameReference.getBoard().getSquareList().get(i));
							while(upperneighbour.isTaken() && (!upperneighbour.isWihthinColumnWord())) {
							...
							while((lowerneighbour.isTaken()) && (!lowerneighbour.isWihthinColumnWord())) {
					...
					if(rightneighbour!=null) {
						if(rightneighbour.isTaken() && (!this.gameReference.getBoard().getSquareList().get(i).isWithinRowWord())) {
							Word word = new Word();
							buffer = new StringBuffer();
							ArrayList<Square> squares = new ArrayList<Square>();
							buffer.append(this.gameReference.getBoard().getSquareList().get(i).getTile().getLetter());
	...
	public void makeMove() {
		...
		gameReference.setPlacedSquares(null);
		for(Square s : squares) {
			this.gameReference.getBoard().placeTile(s.getX(), s.getY(), s.getTile());
		}
		gameReference.setTurnValue(this.turnValue);
