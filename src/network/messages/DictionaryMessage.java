...
public class DictionaryMessage extends Message {
  ...
  /**
   * Constructor: creating a DictionaryMessage with the MessageType.DICTIONARY, the player
   * object of the creator and the user dictionary that should be sent to the other clients.
   * 
   * @author jluellig
   * @param p player object that has sent the message
   * @param dictionary user dictionary that should be sent
   */
  public DictionaryMessage(Player p, HashMap<Character, HashMap<Character, SortedSet<String>>> dictionary) {
	  super(MessageType.DICTIONARY, p);
