...
/**
 * This class is in charge of everything related to the PlayerDB. It provides
 * methods to create new inputs, read data etc.
 *
 * @author jluellig
 */
class Database {
	...
	private static void createTables() {
		try {
			...
			stm.executeUpdate("CREATE TABLE IF NOT EXISTS Statistics (PlayerID INTEGER NOT NULL, Date TEXT NOT NULL, "
	...
	/**
	 * Changes the username of the player with the given ID in the table PlayerInfo
	 * of PlayerDB
	 *
	 * @param username
	 * @param ID
	...
	/**
	 * Changes the avatar of the player with the given ID in the table PlayerInfo of
	 * PlayerDB
	 *
	 * @param avatar
	 * @param ID
	...
	/**
	 * Gives back the ID, username and avatar of each player from the table
	 * PlayerInfo in PlayerDB. It returns a HashMap that uses the player's ID as the
	 * key and the player Data in a String Array as the value. Index 0 marks the
	 * username in the Array, while index 1 stands for the avatar.
	 *
	 * @return playerInfo
	 * @author jluellig
	 */
	...
	/**
	 * Adds statistics (Win, Points) for one game to the given player ID into
	 * Statistics table in PlayerDB
	 *
	 * @param ID
	 * @param win
	 * @param points
	...
	protected static void addStatistics(int ID, int win, int points) {
		try {
			...
			stm.executeUpdate("INSERT INTO Statistics(PlayerID, Date, Win, Points) VALUES(" + ID + ", "
					+ "datetime('now', 'localtime'), " + win + ", " + points + ")");
		} catch (Exception e) {
			System.err.println(e.getClass().getName() + ": " + e.getMessage());
		}
	}
	...
	/**
	 * Gives the statistics of the given player ID from table Statistics in PlayerDB
	 * in a HashMap. Keys: Matches, Won, PointsAVG
	 *
	 * @param ID
	 * @return
	 * @author jluellig
